...
def gen_new_md(accepted, papers):
    ...
    for new_id, x in enumerate(to_print, start=1):
        ...
        print(f'* #{new_id}: {paper} ({name}) ([Paper PDF](prl2021/papers/PRL2021_paper_{num}.pdf)) ([Poster PDF](prl2021/posters/PRL2021_poster_{num}.pdf))')

# Main
with open('accepted-num.txt') as accepted:
    with open('papers-easychair.txt') as papers:
